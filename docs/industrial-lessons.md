...
For most of the twentieth century, a new garment shipped with a small paper slip: *"Inspected by No. 7."* A human had examined it — seams, finish, and above all that no needle remained in the fabric, a check Japanese apparel still treats as non-negotiable (hence the *kenshinki*, 検針機 — needle-detection machines at the end of every line). The slip was a personal certificate. Trust attached to a named inspector, one artifact at a time.

A modern CPU never meets a human inspector. Nobody visually checks a die with tens of billions of transistors — nobody *could*. Every unit passes through automated test equipment: wafer probe before packaging, final test after, thousands of parameters per unit, zero eyeballs. Coverage is 100% *because* it is automated — the semiconductor industry never faced a choice between inspecting everything and inspecting affordably. Trust attaches to the **test program**; the human role is to write, maintain, and calibrate that program.
